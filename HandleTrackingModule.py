...
class handDetector():
    ...
    def findHands(self,img,draw=True):
        ...
        #process the image and get land marks
        self.results = self.hands.process(imgRGB)
        #print(results)
        if self.results.multi_hand_landmarks:
            #loop throigh all the landmarks
            for handles in self.results.multi_hand_landmarks:
                # draw connections of hand landmarks
                if draw:
    ...
    def findPosition(self,img,handNo=0, draw=True):
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id, lm in enumerate(myhand.landmark):
                # print(id,lm)
                h, w, c = img.shape
                # get coordinate position of the finger
                cx, cy = int(lm.x * w), int(lm.y * h)
                xList.append(cx)
                yList.append(cy)
                self.lmList.append([id, cx, cy])
                if draw:
                    cv2.circle(img, (cx, cy), 12, (244, 0, 255), cv2.FILLED)
